import pandas as pd
import os
from util import calc_distance, calc_distance_rank, sideline_dist, defender_in_front, calc_avg_bc_dist, calc_rel_vec_mag, is_dlineman, is_linebacker, is_secondary, is_pass, is_rush, is_bc_wr, is_bc_te, is_bc_rb, is_bc_qb, endzone_dist
...
for game_id in tracking_final.gameId.unique():
    for play_id in tracking_final[tracking_final.gameId==game_id].playId.unique():
        ...
        current_play['rel_vec'] = calc_rel_vec_mag(current_play)
...
data_processed.to_csv('data_features')
